 ...
 * @param isDev - Whether to include development features like console interception (defaults to __DEV__)
 * @returns JavaScript code as a string to be injected into the WebView
 */
export function getInjectedJavaScript(webViewPublicKey: string, browserInfo: BrowserInfo, isDev: boolean = __DEV__): string {
  const consoleInterceptCode = isDev ? `
