...
- `/__tests__/`: Test suites
  - `/lib/webview-injected.test.ts`: WebView API injection tests (signature verification, XSS protection, API functionality)
...
- **Minified JavaScript Injection** (`/lib/webview-injected.ts`):
  - Injected JavaScript is pre-minified at build time using esbuild (59% smaller)
  - Raw templates: `webview-injected.raw.js` and `webview-console-intercept.raw.js`
  - Minified outputs: `webview-injected.min.ts` and `webview-console-intercept.min.ts` (auto-generated)
